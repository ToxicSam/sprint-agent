...
import { getRetroItems, createRetroItem, voteRetroItem, deleteRetroItem, getRetroRatings, createRetroRating } from '@/api/retro';
...
export default function Retro() {
  ...
  const handleRate = useCallback((dimension: string, value: number) => {
    ...
    if (sprint?.id) {
      createRetroRating({ sprint_id: sprint.id, dimension, score: value } as any).catch((err) =>
  ...
  const addItem = useCallback(
    async (text: string) => {
      ...
      if (!trimmed || !sprint?.id) return;
      try {
        const created = await createRetroItem({
          sprint_id: sprint.id,
    ...
    [activeCategory, sprint?.id]
  );

  const handleVote = useCallback(async (id: string) => {
    try {
      const updated = await voteRetroItem(id);
      setItems((prev) =>
        prev.map((item) =>
          item.id === id
            ? { ...item, votes: updated.votes, votedByMe: !item.votedByMe }
            : item
        )
      );
    } catch (err) {
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      ...
      console.error('Failed to delete retro item:', err);
    }
  }, []);
  ...
  const handleGenerateSummary = useCallback(async () => {
    ...
    try {
      if (sprint?.id) {
        const report = await getRetroRatings(sprint.id) as any;
        ...
        if (report.liked?.length) {
          text += `### What Went Well\n`;
          report.liked.forEach((i: any) => { text += `- ${i.text || i.content}\n`; });
          text += `\n`;
        }
        if (report.disliked?.length) {
          text += `### What to Improve\n`;
          report.disliked.forEach((i: any) => { text += `- ${i.text || i.content}\n`; });
          text += `\n`;
        }
        if (report.action_items?.length) {
          text += `### Action Items\n`;
          report.action_items.forEach((i: any) => { text += `- [ ] ${i.text || i.content}\n`; });
          text += `\n`;
        }
        text += `### Sprint Stats\n`;
        ...
        // Fallback: generate locally if no sprint
        ...
        if (rated.length > 0) {
          text += `### Ratings\n`;
          rated.forEach((r) => { text += `- ${r.label}: ${r.score}/5\n`; });
          text += `\n`;
        }
        if (likedItems.length > 0) { text += `### What Went Well\n`; likedItems.forEach((i) => { text += `- ${i.text}\n`; }); text += `\n`; }
        if (dislikedItems.length > 0) { text += `### What to Improve\n`; dislikedItems.forEach((i) => { text += `- ${i.text}\n`; }); text += `\n`; }
        if (actionItems.length > 0) { text += `### Action Items\n`; actionItems.forEach((i) => { text += `- [ ] ${i.text}\n`; }); text += `\n`; }
        setSummary(text);
      }
    } catch (err) {
  ...
  /* Load retro data from API on mount */
  useEffect(() => {
    if (!sprint?.id) return;
    let cancelled = false;
    (async () => {
      try {
        const apiItems = await getRetroItems(sprint.id);
        if (cancelled) return;
        const mapped: RetroItemData[] = apiItems.map((item: any) => ({
          id: item.id,
        ...
        console.error('Failed to load retro items:', err);
      }
    })();
    return () => { cancelled = true; };
  }, [sprint?.id]);

  /* No more localStorage auto-save — data persists via API */
